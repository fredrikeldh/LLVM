...
//===----------------------------------------------------------------------===//
//
// This file implements the MapipTargetLowering class.
//
//===----------------------------------------------------------------------===//
...
MapipTargetLowering::MapipTargetLowering(TargetMachine &TM)
  ...
  addRegisterClass(MVT::i1,  Mapip::PredsRegisterClass);
  addRegisterClass(MVT::i16, Mapip::RRegu16RegisterClass);
  addRegisterClass(MVT::i32, Mapip::RRegu32RegisterClass);
  addRegisterClass(MVT::i64, Mapip::RRegu64RegisterClass);
  addRegisterClass(MVT::f32, Mapip::RRegf32RegisterClass);
  addRegisterClass(MVT::f64, Mapip::RRegf64RegisterClass);
...
SDValue MapipTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
...
const char *MapipTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
    ...
    case MapipISD::READ_PARAM:
      return "MapipISD::READ_PARAM";
    case MapipISD::EXIT:
      return "MapipISD::EXIT";
    case MapipISD::RET:
      return "MapipISD::RET";
  }
}
...
struct argmap_entry {
  ...
  argmap_entry(MVT::i1,  Mapip::PredsRegisterClass),
  argmap_entry(MVT::i16, Mapip::RRegu16RegisterClass),
  argmap_entry(MVT::i32, Mapip::RRegu32RegisterClass),
  argmap_entry(MVT::i64, Mapip::RRegu64RegisterClass),
  argmap_entry(MVT::f32, Mapip::RRegf32RegisterClass),
  argmap_entry(MVT::f64, Mapip::RRegf64RegisterClass)
};
...
SDValue MapipTargetLowering::
  ...
  if (isVarArg) llvm_unreachable("Mapip does not support varargs");
  ...
  switch (CallConv) {
    default:
      ...
      break;
    case CallingConv::Mapip_Kernel:
      MFI->setKernel(true);
      break;
    case CallingConv::Mapip_Device:
      MFI->setKernel(false);
      break;
  }
  ...
  for (int i = 0, e = Ins.size(); i != e; ++ i) {
    ...
    if (MFI->isKernel() && entry->RC == Mapip::PredsRegisterClass)
    ...
    SDValue inval;
    if (MFI->isKernel())
      inval = DAG.getNode(MapipISD::READ_PARAM, dl, VT, Chain,
...
SDValue MapipTargetLowering::
  ...
  switch (CallConv) {
    ...
    case CallingConv::Mapip_Kernel:
      ...
      return DAG.getNode(MapipISD::EXIT, dl, MVT::Other, Chain);
    case CallingConv::Mapip_Device:
  ...
  // return void
  if (Outs.size() == 0)
    return DAG.getNode(MapipISD::RET, dl, MVT::Other, Chain);
  ...
  return DAG.getNode(MapipISD::RET, dl, MVT::Other, Chain, Flag);
}
