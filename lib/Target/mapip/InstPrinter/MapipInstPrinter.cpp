...
const char* Mapip::MapipFCCToString(Mapip::CondCode CC) {
  switch (CC) {
  case FCOND_F:
...
StringRef MapipInstPrinter::getOpcodeName(unsigned Opcode) const {
  return getInstructionName(Opcode);
}

void MapipInstPrinter::printRegName(raw_ostream &OS, unsigned RegNo) const {
...
void MapipInstPrinter::printUnsignedImm(const MCInst *MI, int opNum,
                                       raw_ostream &O) {
...
printFCCOperand(const MCInst *MI, int opNum, raw_ostream &O) {
  const MCOperand& MO = MI->getOperand(opNum);
  O << MapipFCCToString((Mapip::CondCode)MO.getImm());
}
